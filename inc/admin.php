 ...
 * from the theme, supplied through the `COLOPHON_SLUG . '/get_started_content'` filter
 ...
 */
define( 'COLOPHON_WELCOME_FLAG', 'colophon_welcome_notice' ); // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-post action and nonce action string for dismissing the welcome notice.
 */
define( 'COLOPHON_DISMISS_ACTION', 'colophon_dismiss_welcome' ); // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-menu slug for the Get-started page.
 */
define( 'COLOPHON_GET_STARTED_SLUG', 'colophon-get-started' );
 ...
 */
function colophon_flag_welcome_notice(): void {
	add_option( COLOPHON_WELCOME_FLAG, '1', '', 'no' );
}
add_action( 'after_switch_theme', 'colophon_flag_welcome_notice' );
 ...
 */
function colophon_clear_welcome_notice(): void {
	delete_option( COLOPHON_WELCOME_FLAG );
}

/**
 * Register the "Get started" page under Appearance.
 *
 * The required capability comes from colophon_get_onboarding_capability() — default is
 * edit_theme_options (users who can switch themes and configure global styles),
 * filterable via COLOPHON_SLUG . '/onboarding_capability'.
 */
function colophon_register_get_started_page(): void {
	add_theme_page(
		/* translators: %s: theme name. */
		sprintf( esc_html__( '%s: Get started', 'colophon' ), colophon_get_theme_name() ),
		colophon_get_theme_name(),
		colophon_get_onboarding_capability(),
		COLOPHON_GET_STARTED_SLUG,
		'colophon_render_get_started_page'
	);
}
add_action( 'admin_menu', 'colophon_register_get_started_page' );
 ...
 */
function colophon_get_theme_name(): string {
 ...
 */
function colophon_get_onboarding_capability(): string {
	/**
	 ...
	 */
	return (string) apply_filters( COLOPHON_SLUG . '/onboarding_capability', 'edit_theme_options' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function colophon_enqueue_get_started_assets( string $hook_suffix ): void {
	if ( 'appearance_page_' . COLOPHON_GET_STARTED_SLUG !== $hook_suffix ) {
		return;
	}

	$sheet = COLOPHON_DIR . '/assets/css/admin-get-started.css';
	if ( ! file_exists( $sheet ) ) {
		return;
	}

	wp_enqueue_style(
		COLOPHON_SLUG . '-get-started',
		COLOPHON_URI . '/assets/css/admin-get-started.css',
		array(),
...
function colophon_render_welcome_notice(): void {
	...
	if ( '1' !== get_option( COLOPHON_WELCOME_FLAG ) ) {
		return;
	}

	$screen = get_current_screen();
	if ( $screen instanceof \WP_Screen && 'appearance_page_' . COLOPHON_GET_STARTED_SLUG === $screen->id ) {
		return;
	}

	$page_url = admin_url( 'themes.php?page=' . COLOPHON_GET_STARTED_SLUG );
...
add_action( 'admin_notices', 'colophon_render_welcome_notice' );
 ...
 */
function colophon_handle_welcome_dismiss(): void {
	if ( ! current_user_can( colophon_get_onboarding_capability() ) ) {
		wp_die( esc_html__( 'You do not have permission to do that.', 'colophon' ) );
	}

	check_admin_referer( COLOPHON_DISMISS_ACTION );
	colophon_clear_welcome_notice();
	wp_safe_redirect( admin_url() );
	exit;
}
add_action( 'admin_post_' . COLOPHON_DISMISS_ACTION, 'colophon_handle_welcome_dismiss' );

/**
 * The Get-started page content.
 *
 * Core supplies a plain, honest default so the page is coherent out of the box.
 * A theme overrides any part of it through the `COLOPHON_SLUG . '/get_started_content'`
...
function colophon_get_started_content(): array {
	...
	$default = array(
		'lead'       => sprintf(
			/* translators: %s: theme name. */
			esc_html__( '%s is a free, full-site-editing theme built to get out of the way of your content. Here is how to make it yours.', 'colophon' ),
			$theme
		),
		/*
		 ...
		 * echo in colophon_render_get_started_page() (the 'title' at the <strong>, the
		...
		'steps'      => array(
			array(
				...
				'body'  => esc_html__( "A visitor's first second decides whether they stay. Set a static front page under Settings → Reading, and give your posts a page of their own.", 'colophon' ),
			),
			array(
			...
			array(
				...
				'body'  => esc_html__( 'In any page or post, open the block inserter, choose Patterns, and find this theme\'s group. Drop one in and change the words.', 'colophon' ),
			),
			array(
	 ...
	 *
	 * A theme replaces any key with its own voice. See colophon_get_started_content() for
	 * the array shape.
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	return (array) apply_filters( COLOPHON_SLUG . '/get_started_content', $default ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function colophon_render_get_started_page(): void {
	if ( ! current_user_can( colophon_get_onboarding_capability() ) ) {
		return;
	}
	...
	$theme   = colophon_get_theme_name();
	$content = colophon_get_started_content();
