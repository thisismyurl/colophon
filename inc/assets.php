 ...
 *
 * Portable: every handle derives from the COLOPHON_SLUG constant, so a re-skin needs no
 ...
 *
 * Cache-buster is each file's modification time, not the COLOPHON_VERSION constant, so a
 * CSS edit busts caches immediately during iteration without a version bump.
 */
function colophon_enqueue_assets(): void {
...
add_action( 'wp_enqueue_scripts', 'colophon_enqueue_assets' );
 ...
 * an off-origin fetch.
 */
function colophon_preload_fonts(): void {
	/**
	 ...
	 * @since 1.0.0
	 */
	$fonts = (array) apply_filters( COLOPHON_SLUG . '/preload_fonts', array() );
	...
	foreach ( $fonts as $font ) {
		...
		printf(
			...
			esc_url( COLOPHON_URI . '/' . ltrim( $font, '/' ) )
		);
	}
}
add_action( 'wp_head', 'colophon_preload_fonts', 1 );
