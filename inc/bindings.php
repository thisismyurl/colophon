...
function colophon_register_bindings(): void {
	...
	register_block_bindings_source(
		COLOPHON_SLUG . '/site-strings',
		array(
			'label'              => esc_html__( 'Theme strings', 'colophon' ),
			'get_value_callback' => 'colophon_get_site_string_value',
			'uses_context'       => array(),
		)
	);

	register_block_bindings_source(
		COLOPHON_SLUG . '/publication-date',
		array(
			'label'              => esc_html__( 'Publication date', 'colophon' ),
			'get_value_callback' => 'colophon_get_publication_date_value',
			'uses_context'       => array(),
		)
	);
	...
	register_block_bindings_source(
		...
		array(
			'label'              => esc_html__( 'Footer credit', 'colophon' ),
 ...
 */
function colophon_get_site_string_value( array $source_args ): string {
	switch ( $source_args['key'] ?? '' ) {
		case 'breaking-label':
			return esc_html__( 'Breaking', 'colophon' );
		case 'home-label':
			return esc_html__( 'Back to the front page', 'colophon' );
		case 'home-url':
			return esc_url( home_url( '/' ) );
		case 'not-found-title':
			return esc_html__( 'Page not found', 'colophon' );
		case 'posts-title':
			$posts_page = (int) get_option( 'page_for_posts' );
			return $posts_page ? esc_html( get_the_title( $posts_page ) ) : esc_html__( 'Latest news', 'colophon' );
	}
	return '';
}
...
function colophon_get_copyright_value(): string {
	 ...
	 */
	$format = (string) apply_filters( COLOPHON_SLUG . '/copyright_date_format', 'Y' );
	 ...
	 */
	$copyright = (string) apply_filters( COLOPHON_SLUG . '/copyright_text', $copyright );
...
function colophon_get_footer_credit_value(): string {
	...
	$text = sprintf(
		...
		esc_html__( 'Built with %s.', 'colophon' ),
		$credit
	);
	 ...
	 */
	$text = (string) apply_filters( COLOPHON_SLUG . '/footer_credit_text', $text ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
 ...
 */
function colophon_get_publication_date_value(): string {
	/**
	 ...
	 */
	$format = (string) apply_filters( COLOPHON_SLUG . '/publication_date_format', (string) get_option( 'date_format' ) ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
